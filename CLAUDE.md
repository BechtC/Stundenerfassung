...
Persoenliches Stundenerfassungs-Tool mit Rechnungsstellung. Streamlit-Web-App mit SQLite-Backend, laeuft lokal im Chrome App-Mode auf Port 8502.
...
# Testdaten generieren (einmalig)
python testdaten.py
```

## Architecture

- **app.py** — Streamlit-Hauptanwendung mit 5 Seiten (Zeiterfassung, Dashboard, Projekte, Rechnungen, Einstellungen). Nutzt `st.session_state.positionen` fuer den Rechnungs-Workflow.
...
- Rechnungsnummern-Format: `RE-{JAHR}-{NNNN}` (auto-increment)
- Kleinunternehmerregelung (§19 UStG) als Default in PDFs
